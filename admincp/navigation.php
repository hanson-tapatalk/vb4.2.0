...
if ($_REQUEST['do'] == 'update')
{
	...
	foreach($active AS $value => $keys)
	{
		$sql = "
			UPDATE " . TABLE_PREFIX . "navigation
			SET state = (state | " . NAV_EDITED . ") " .
			($value ? '| ' : ' & ~') . NAV_ACTIVE . " 
			WHERE (state & " . NAV_ACTIVE . ") != " .
			($value ? NAV_ACTIVE : 0) . "
			AND navid IN (" . implode(',',$keys) . 
		")";
...
if ($_REQUEST['do'] == 'doedit')
{
	...
	if (!$vbulletin->GPC['title'] OR strlen($vbulletin->GPC['title']) > 50)
	{
	...
	if ($edited)
	{
		$sql = "
			UPDATE " . TABLE_PREFIX . "navigation
			SET state = ((state | " . NAV_EDITED . ") " . 
			($vbulletin->GPC['active'] ? '| ' : ' & ~') . NAV_ACTIVE . ") " . 
			($vbulletin->GPC['usetabid'] ? '| ' : ' & ~') . NAV_USETABID . ",
...
if ($_REQUEST['do'] == 'doadd')
{
	...
	$sqldata = '(' .
		...
		''	. ($vbulletin->GPC['active'] ? NAV_ACTIVE : 0) . ' | ' . // OR ...
		''	. ($vbulletin->GPC['usetabid'] ? NAV_USETABID : 0) . ',' .
		''	. $vbulletin->GPC['order'] .',' .
...
if ($_REQUEST['do'] == 'dodefault')
{
	...
	if ($vbulletin->GPC['confirm'])
	{
		...
		$sql = "
			UPDATE " . TABLE_PREFIX . "navigation
			SET state = (state | " . NAV_EDITED . ") & ~" . NAV_DEFAULT . "
			$sqlset1
